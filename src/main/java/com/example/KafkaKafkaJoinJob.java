...
public class KafkaKafkaJoinJob {
    public static void main(String[] argv) throws Exception {
        System.out.println("=====[KafkaKafkaJoin (join) Job started]=====");
        ...
        DataStream<Snipe> enrichedStream = clickStream
                .join(usersStream)
                .where(new KeySelector<ClickStreamEvent, String>() {
                    @Override
                    public String getKey(ClickStreamEvent value) throws Exception {
                ...
                .equalTo(new KeySelector<User, String>() {
                    ...
                    public String getKey(User value) throws Exception {
                        return value.getUserid(); // Join key
                    }
                })
                .window(TumblingProcessingTimeWindows.of(Time.seconds(10))) // Window size
                .apply(new JoinFunction<ClickStreamEvent, User, Snipe>() {
                    @Override
                    public Snipe join(ClickStreamEvent clickStreamEvent, User user) throws Exception {
                        return Snipe.newInstance(clickStreamEvent, user);
                    }
                });
